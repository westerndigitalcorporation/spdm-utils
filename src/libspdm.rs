...
///
/// Ok(()) on success
pub fn setup_transport_layer(context: *mut c_void) -> Result<(), ()> {
    unsafe {
        ...
        let libspdm_scratch_buffer_ptr: *mut c_void =
            libspdm_scratch_buffer as *mut _ as *mut c_void;
...
pub fn initialise_connection(context: *mut c_void, slot_id: u8) -> Result<(), u32> {
    unsafe {
    ...
    unsafe {
        ...
        let total_digest_buffer_ptr: *mut c_void =
            &mut total_digest_buffer as *mut _ as *mut c_void;
...
/// Ok(()) on success
/// Err(ret), where ret is a libspdm return status indicating an error.
pub fn get_measurement(context: *mut c_void, slot_id: u8) -> Result<(), u32> {
    unsafe {
        let request_attribute: u8 =
            SPDM_GET_MEASUREMENTS_REQUEST_ATTRIBUTES_GENERATE_SIGNATURE as u8;
        ...
        for i in 1..0xFF {
            ...
            let measurement_record_ptr: *mut c_void =
                &mut measurement_record as *mut _ as *mut c_void;
...
pub unsafe extern "C" fn libspdm_requester_data_sign(
    _spdm_version: spdm_version_number_t,
    _op_code: u8,
...
pub unsafe extern "C" fn libspdm_responder_data_sign(
    spdm_version: spdm_version_number_t,
    op_code: u8,
    base_asym_algo: u32,
    base_hash_algo: u32,
    is_data_hash: bool,
    ...
    match measurement_summary_hash_type as u32 {
        ...
        | SPDM_CHALLENGE_REQUEST_ALL_MEASUREMENTS_HASH => {
            ...
            for _i in 0..device_measurement_count {
                let cached_measurment_block: *mut spdm_measurement_block_dmtf_t =
...
unsafe fn libspdm_fill_measurement_image_hash_block(
    ...
    measurement_block: *mut spdm_measurement_block_dmtf_t,
) -> size_t {
    let data = [measurements_index; LIBSPDM_MEASUREMENT_RAW_DATA_SIZE as usize];
    let hash_size = libspdm_get_measurement_hash_size(measurement_hash_algo) as usize;

    (*measurement_block).measurement_block_common_header.index = measurements_index;
    (*measurement_block)
        .measurement_block_common_header
        .measurement_specification = SPDM_MEASUREMENT_SPECIFICATION_DMTF as u8;
...
unsafe fn libspdm_fill_measurement_svn_block(
    ...
    let svn: spdm_measurements_secure_version_number_t = 0x7;
    ...
    (*measurement_block)
        ...
        core::mem::size_of::<spdm_measurements_secure_version_number_t>() as u16;
    ...
    (measurement_block.add(1) as *mut u8).copy_from(
        ...
        core::mem::size_of::<spdm_measurements_secure_version_number_t>(),
    );

    core::mem::size_of::<spdm_measurement_block_dmtf_t>()
        + core::mem::size_of::<spdm_measurements_secure_version_number_t>()
}
...
unsafe fn libspdm_fill_measurement_manifest_block(
    ...
    if use_bit_stream {
        ...
        core::mem::size_of::<spdm_measurement_block_dmtf_t>() + size
    } else {
        (*measurement_block)
            .measurement_block_dmtf_header
            .dmtf_spec_measurement_value_type =
...
unsafe fn libspdm_fill_measurement_device_mode_block(
    ...
    let device_mode = spdm_measurements_device_mode_t {
    ...
    (*measurement_block)
        .measurement_block_dmtf_header
        .dmtf_spec_measurement_value_size =
        core::mem::size_of::<spdm_measurements_device_mode_t>() as u16;

    (*measurement_block)
        .measurement_block_common_header
        .measurement_size = (core::mem::size_of::<spdm_measurement_block_dmtf_header_t>()
        + core::mem::size_of::<spdm_measurements_device_mode_t>())
        as u16;
...
pub unsafe extern "C" fn libspdm_measurement_collection(
    ...
    if u32::from(measurement_specification) != SPDM_MEASUREMENT_SPECIFICATION_DMTF
        || measurement_hash_algo == 0
    {
        return libspdm_status_construct!(LIBSPDM_SEVERITY_ERROR, LIBSPDM_SOURCE_CORE, 0x0002);
    }

    let hash_size = libspdm_get_measurement_hash_size(measurement_hash_algo);
    ...
    match u32::from(measurements_index) {
        ...
        SPDM_GET_MEASUREMENTS_REQUEST_MEASUREMENT_OPERATION_ALL_MEASUREMENTS => {
            ...
            for i in 1..(LIBSPDM_MEASUREMENT_BLOCK_HASH_NUMBER as u8 + 1) {
                let measurement_block = measurement_ptr as *mut spdm_measurement_block_dmtf_t;
            ...
            let mut total_size_needed = if !use_bit_stream {
                LIBSPDM_MEASUREMENT_BLOCK_HASH_NUMBER
                    * (core::mem::size_of::<spdm_measurement_block_dmtf_t>() as u32 + hash_size)
            } else {
                LIBSPDM_MEASUREMENT_BLOCK_HASH_NUMBER
                    * (core::mem::size_of::<spdm_measurement_block_dmtf_t>() as u32
                        + LIBSPDM_MEASUREMENT_RAW_DATA_SIZE)
            };
            /* Next one - SVN is always raw bitstream data.*/
            total_size_needed += core::mem::size_of::<spdm_measurement_block_dmtf_t>() as u32
                + core::mem::size_of::<spdm_measurements_secure_version_number_t>() as u32;
            /* Next one - manifest is always raw bitstream data.*/
            total_size_needed += measurement_block_size;
            /* Next one - device_mode is always raw bitstream data.*/
            total_size_needed += core::mem::size_of::<spdm_measurement_block_dmtf_t>() as u32
                + core::mem::size_of::<spdm_measurements_device_mode_t>() as u32;
    ...
    if !content_changed.is_null()
        && ((u32::from(spdm_version) >> SPDM_VERSION_NUMBER_SHIFT_BIT) >= SPDM_MESSAGE_VERSION_12)
    {
        ...
        if (u32::from(request_attribute)
            ...
            != 0
        {
            *content_changed = SPDM_MEASUREMENTS_RESPONSE_CONTENT_NO_CHANGE_DETECTED as u8;
        } else {
            *content_changed = SPDM_MEASUREMENTS_RESPONSE_CONTENT_CHANGE_NO_DETECTION as u8;
        }
    }
...
pub unsafe extern "C" fn libspdm_psk_handshake_secret_hkdf_expand(
    ...
    let hash_size = libspdm_get_hash_size(base_hash_algo) as usize;
    let ret = libspdm_hkdf_extract(
        base_hash_algo,
    ...
    libspdm_zero_mem(&mut handshake_secret as *mut _ as *mut c_void, hash_size);
...
pub unsafe extern "C" fn libspdm_psk_master_secret_hkdf_expand(
    ...
    libspdm_zero_mem(handshake_secret.as_mut_ptr() as *mut c_void, hash_size);
    ...
    libspdm_zero_mem(salt1.as_mut_ptr() as *mut c_void, hash_size);
    ...
    libspdm_zero_mem(master_secret.as_mut_ptr() as *mut c_void, hash_size);
    ret
}
...
pub(crate) unsafe fn get_local_certchain(
    path: &Path,
    _asym_algo: u32,
    hash_algo: u32,
    ...
    let digest_size = libspdm_get_hash_size(hash_algo) as usize;

    let cert_chain_size = core::mem::size_of::<spdm_cert_chain_t>() + digest_size + buffer_len;
    let cert_chain_layout = Layout::from_size_align(cert_chain_size, 8).unwrap();
    let cert_chain_buffer = alloc(cert_chain_layout);
    let cert_chain = cert_chain_buffer as *mut spdm_cert_chain_t;
    ...
    if !libspdm_hash_all(
        ...
        root_cert_size,
        cert_chain_buffer.add(core::mem::size_of::<spdm_cert_chain_t>()),
    ) {
        panic!("Unable to hash data");
    }

    let cert_buffer_location =
        cert_chain_buffer.add(core::mem::size_of::<spdm_cert_chain_t>() + digest_size);
...
pub fn get_negotiated_algos(cntx_ptr: *mut c_void, slot_id: u8) -> Result<(), u32> {
    unsafe {
        debug!("The following algorithms were negotiated with the endpoint:");
        let parameter = libspdm_data_parameter_t::new_connection(slot_id);

        let mut spdm_algo_base_asym = libspdm::SpdmAlgoBaseAsym(0);
        let asym_dptr = &mut spdm_algo_base_asym.0 as *mut _ as *mut c_void;

        let mut spdm_algo_base_hash = libspdm::SpdmAlgoBaseHash(0);
        let hash_dptr = &mut spdm_algo_base_hash.0 as *mut _ as *mut c_void;

        let mut spdm_dhe = libspdm::SpdmAlgoDheNamedGroup(0);
        let dhe_dptr = &mut spdm_dhe.0 as *mut _ as *mut c_void;

        let mut spdm_aead_cipher = libspdm::SpdmAlgoAeadCipherSuite(0);
        let aead_dptr = &mut spdm_aead_cipher.0 as *mut _ as *mut c_void;

        let mut spdm_key_sched = libspdm::SpdmAlgoKeyScheduleHmacHash(0);
...
pub unsafe fn requester_respond_if_ready(
    ...
    let context = cntx_ptr as *mut libspdm_context_t;
    ...
    let ret = libspdm_acquire_sender_buffer(
        cntx_ptr as *mut libspdm_context_t,
    ...
    let spdm_version: u8 = libspdm_get_connection_version(context);

    let spdm_req = (*msg_ptr_ptr).add(transport_header_size);
    let spdm_req_ptr = spdm_req as *mut spdm_response_if_ready_request_t;
    (*spdm_req_ptr).header.spdm_version = spdm_version;
    (*spdm_req_ptr).header.request_response_code = u8::try_from(SPDM_RESPOND_IF_READY).unwrap();
    ...
    let spdm_request_size = core::mem::size_of::<spdm_response_if_ready_request_t>();

    let ret = libspdm_send_spdm_request(
        cntx_ptr as *mut libspdm_context_t,
        session_id_ptr,
        spdm_request_size,
        spdm_req_ptr as *mut _ as *mut c_void,
    );
    libspdm_release_sender_buffer(cntx_ptr as *mut libspdm_context_t);
    if LibspdmReturnStatus::libspdm_status_is_error(ret) {
        return Err(ret);
    }
    ...
    let ret = libspdm_acquire_receiver_buffer(
        cntx_ptr as *mut libspdm_context_t,
    ...
    libspdm_zero_mem(*response_ptr_ptr, response_size);

    let ret = libspdm_receive_spdm_response(
        cntx_ptr as *mut libspdm_context_t,
        session_id_ptr,
    ...
    let spdm_response = (*response_ptr_ptr) as *mut spdm_message_header_t;

    if response_size < core::mem::size_of::<spdm_message_header_t>() {
        return Err(libspdm::LIBSPDM_STATUS_INVALID_MSG_SIZE);
    }

    if (*spdm_response).spdm_version != spdm_version {
        return Err(libspdm::LIBSPDM_STATUS_INVALID_MSG_FIELD);
    }

    if u32::try_from((*spdm_response).request_response_code).unwrap() == SPDM_ERROR {
        // TODO: This request will typically fail here, as
        // SPDM-Utils responder is not expecting a RespondIfReady.
        let ret = libspdm_handle_simple_error_response(context, (*spdm_response).param1);
        return Err(ret);
    }
