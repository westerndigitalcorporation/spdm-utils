...
impl SgCmd {
    ...
    /// # Summary
    ///
    /// Generate an IF-SEND/SECURITY_IN command to send an SPDM request.
    ///
    /// # Returns
    ///
    /// Ok(SgCmd), An initialised command structure
    ...
    fn gen_spdm_response(
        ...
        sg_cmd.cmdp[1..(header_len + 1)].copy_from_slice(&transp_msg_buf[0..header_len]);
...
unsafe extern "C" fn scsi_send_message(
    ...
    let mut cmd = SgCmd::gen_spdm_request(
        ...
        SG_DXFER_TO_DEV,
        SG_BUF_MAX_SIZE,
        &msg_buf[0..header_len].try_into().unwrap(),
    )
    .unwrap();
...
unsafe extern "C" fn scsi_receive_message(
    ...
    let mut cmd = SgCmd::gen_spdm_response(
        0,
        SG_DXFER_FROM_DEV,
        SG_BUF_MAX_SIZE,
        message,
    ...
    // Get the size from the SCSI header
    *message_size = u32::from_be_bytes(msg_buf[5..9].try_into().unwrap()) as usize - 1;
