...
pub fn setup_capabilities(
    ...
    unsafe {
        ...
        let cert_chain: &'static mut [u8] = Box::leak(
    ...
    unsafe {
        match code {
            ...
            RequestCode::EncapsulatedSendReceive { secure_msg } => {
                if secure_msg && session_info.session_id == 0 {
                    error!("Cannot do secure messsages with no session id established");
                    return Err(LIBSPDM_STATUS_INVALID_STATE_LOCAL);
                }
            ...
            RequestCode::SetCertificate {} => {
                ...
                let mut cert_chain = get_local_certchain(buffer, asym_algo, hash_algo, true);
...
pub fn setup_pqc_requester_cert_chain(
    ...
    let cert_chain: &'static mut [u8] =
        Box::leak(get_local_certchain(&buffer, 0, hash_algo, true).into_boxed_slice());
    let parameter = libspdm_data_parameter_t::new_local(slot_id);
