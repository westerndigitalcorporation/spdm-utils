...
unsafe extern "C" fn usb_i2c_send_message(
    ...
    let mut send_buf: [u8; (SEND_RECEIVE_BUFFER_LEN + 4) as usize] =
        [0; (SEND_RECEIVE_BUFFER_LEN + 4) as usize];
...
unsafe extern "C" fn usb_i2c_receive_message(
    ...
    // Note: This is to address an odd quirk, where the after usb-i2c device is newly
    // flashed (first time), the first transfer from the device to us may have an
    // odd offset, as if the TX buffer is non-empty (perhaps the end of the board boot banner
    // lingering in the internal buffers).
    loop {
        if header
